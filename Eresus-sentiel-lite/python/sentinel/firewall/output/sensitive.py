...
- YAML-driven regex patterns (rules/output_sensitive_patterns.yaml)
...
def _load_pii_patterns() -> dict:
    """Load PII/secret patterns from YAML rules file."""
    patterns: dict = {}
    try:
        with open(_PATTERNS_FILE) as f:
            raw = yaml.safe_load(f)
        if not isinstance(raw, list):
            logger.warning("output_sensitive_patterns.yaml: expected list, got %s", type(raw).__name__)
            return patterns
        for entry in raw:
            ...
            try:
                ...
                logger.warning("Bad regex in pattern %s: %s", pid, e)
                continue
            patterns[pid] = {
                "pattern": compiled,
                "description": entry.get("description", pid),
                "severity": _SEVERITY_MAP.get(entry.get("severity", "MEDIUM"), Severity.MEDIUM),
                "category": entry.get("category", "pii"),
            }
        logger.info("Loaded %d output-sensitive patterns from %s", len(patterns), _PATTERNS_FILE.name)
    except FileNotFoundError:
        logger.warning("Pattern file not found: %s — using empty set", _PATTERNS_FILE)
    except Exception as e:
        logger.warning("Failed to load output_sensitive_patterns.yaml: %s", e)
    return patterns
