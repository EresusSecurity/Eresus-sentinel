...
def create_dashboard_app(
    ...
    configured_user = os.environ.get("SENTINEL_USER", "admin")
    configured_password = os.environ.get("SENTINEL_PASSWORD", "")

    from sentinel.db import init_db, is_db_enabled
    from sentinel.db_store import DBStatsStore
    ...
    async def api_login(body: LoginRequest):
        if not configured_password:
            raise HTTPException(
                status_code=503,
                detail="SENTINEL_PASSWORD is not configured on the server",
            )

        valid_user = secrets.compare_digest(body.username, configured_user)
    ...
    async def api_stats():
        if db_store:
            return await db_store.get_stats()
    ...
    async def artifact_scan(file: UploadFile = File(...)):
        ...
        try:
            ...
            if db_store:
                await db_store.record_artifact(
                    filename=safe_name,
                    findings=findings,
                    latency_ms=elapsed,
                    size=len(content),
    ...
    async def api_history():
        if db_store:
            return await db_store.get_history()
        return {
