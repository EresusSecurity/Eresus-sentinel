...
class TestSASTAnalyzer(unittest.TestCase):
    ...
    def test_safe_literal_eval_no_finding(self):
        """ast.literal_eval() is safe and should NOT trigger SAST-020."""
        path = self._write_temp_py("import ast\ndata = ast.literal_eval(user_input)")
        findings = self.analyzer.scan_path(path)
        os.unlink(path)
        rule_ids = [f.rule_id for f in findings]
        self.assertNotIn("SAST-020", rule_ids)

    def test_multiple_rules_per_line(self):
        """Multiple rules should fire on the same line (no first-rule-wins)."""
        path = self._write_temp_py("eval(pickle.load(f))")
        findings = self.analyzer.scan_path(path)
        os.unlink(path)
        rule_ids = [f.rule_id for f in findings]
