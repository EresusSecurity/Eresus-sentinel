...
1. **Auth and rate limiting** — API token validated; per-IP and per-session rate limits checked
2. **Session ID derived** — HMAC-SHA256 derived from auth token, IP, and namespace — callers cannot predict or forge session IDs
...
| 90–100 | `block` | Nothing sent to LLM; synthetic refusal returned |

### Session escalation

Once a session's cumulative score crosses `SESSION_DECEIVE_THRESHOLD` (default 300), any subsequent query scoring > 0 is forced to `deceive` — even queries that would normally only reach `warn`.

Queries scoring exactly 0 always pass normally regardless of session history. Hard `block` is never triggered by session history alone.
...
| Redis (if configured) | Session metadata + DECEIVE responses (capped 4,096 chars) | 24 h TTL |

The LLM provider receives exactly the same data it would receive if you called it directly. Eresus Sentinel does not add new data recipients.
...
The proxy response shape differs from raw LLM provider format.
...
Your application      ← formats its own API responses; no raw proxy exposure
   │
   ▼
Eresus Sentinel       ← SENTINEL_ENV=production, internal network only
   │
   ▼
LLM Provider
```
