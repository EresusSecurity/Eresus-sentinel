...
class RelaxedZipFile:
    ...
    def open(cls, source: Union[str, Path, bytes]) -> "RelaxedZipFile":
        ...
        if len(data) > MAX_SCAN_FILE_SIZE:
            raise FileTooLargeError(str(source), len(data), MAX_SCAN_FILE_SIZE)
        return cls(data)
    ...
    def _read_entry(self, entry: ZipEntry) -> bytes:
        ...
        if entry.compression_method == 0:
            ...
            # Deflate — bounded decompression to prevent zip bombs
            import zlib
            try:
                return zlib.decompress(raw, -15, MAX_DECOMPRESS_SIZE)
            except zlib.error:
