...
def emit_global(buf: io.BytesIO, state: PVMState, rng: random.Random,
                safe_only: bool = False) -> None:
    mod, attr = get_random_safe_global(rng) if safe_only else get_random_global(rng)
    buf.write(b"\x63")
    buf.write(f"{mod}\n{attr}\n".encode())
...
def emit_inst(buf: io.BytesIO, state: PVMState, rng: random.Random,
              safe_only: bool = False) -> None:
    mod, attr = get_random_safe_global(rng) if safe_only else get_random_global(rng)
    buf.write(b"\x69")
    buf.write(f"{mod}\n{attr}\n".encode())
