...
class SandboxExecutor:
    ...
    def _apply_resource_limits(self) -> None:
        try:
            import resource
            if self._policy.max_memory_mb > 0:
                mem_bytes = self._policy.max_memory_mb * 1024 * 1024
                resource.setrlimit(resource.RLIMIT_AS, (mem_bytes, mem_bytes))
            if self._policy.max_cpu_seconds > 0:
                cpu_sec = int(self._policy.max_cpu_seconds)
                resource.setrlimit(resource.RLIMIT_CPU, (cpu_sec, cpu_sec + 5))
            if self._policy.max_open_files > 0:
                resource.setrlimit(
                    resource.RLIMIT_NOFILE,
                    (self._policy.max_open_files, self._policy.max_open_files),
                )
            if self._policy.max_processes > 0:
                resource.setrlimit(
                    resource.RLIMIT_NPROC,
                    (self._policy.max_processes, self._policy.max_processes),
                )
        except (ImportError, ValueError, OSError) as e:
            logger.debug("Resource limits not available: %s", e)
